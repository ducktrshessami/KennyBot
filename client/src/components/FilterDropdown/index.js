...
export default forwardRef(function FilterDropdown(props, ref) {
    ...
    return (
        <div className={`filter-dropdown dbnb-bg ${props.users ? "user-filter" : ""} ${props.className ? props.className : ""}`.trim()} ref={ref}>
            ...
            <ul>
                {items.map((item, i) => <FilterItem key={item.value} active={item.value === props.activeValue} primary={item.primary} secondary={item.secondary} image={item.image || stockAvatar} ref={itemRefs[i]} onClick={selectFilter} />)}
            </ul>
        </div>
    );
});
